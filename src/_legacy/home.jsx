...
function StatCard({ icon, count, label, sublabel, onOpen }) {
  const Ic = Icons[icon];
  return (
...
function ActivityChip({ tone, label }) {
  const tones = {
  ...
  return (
    <span className={"chip ring-1 ring-inset " + tones[tone]}>
      <span className={"h-1.5 w-1.5 rounded-full " + (tone === 'mint' ? 'bg-mint-500' : tone === 'amber' ? 'bg-amber-500' : 'bg-accent-500')} />
      {label}
    </span>
  );
}
...
function ActivityRow({ item, idx }) {
  const labelByType = { docs: 'Docs', calendar: 'Calendar', billing: 'Billing', crm: 'CRM', inbox: 'Inbox', calls: 'Calls' };
  return (
    <div className="grid grid-cols-[88px_1fr_auto] items-center gap-4 px-1 py-3 border-b border-ink-800/60 last:border-b-0 hover:bg-ink-850/40 transition rounded">
    ...
    <div className="screen-enter relative ambient">
      <header className="flex items-center justify-between px-10 pt-7 pb-6">
        <div className="flex items-center gap-2.5">
          <span className="text-ink-200"><Icons.Home size={17} /></span>
      ...
      <section className="px-10">
        <div className="grid grid-cols-3 gap-4">
          {STAT_CARDS.map(s => (
            <StatCard key={s.key} {...s} onOpen={() => setRoute(s.key)} />
          ))}
        </div>
      </section>
      ...
      <section className="px-10 pt-10 pb-10">
        ...
        <div className="card-surface rounded-xl px-4 py-1">
          {ACTIVITY.map((a, i) => <ActivityRow key={i} item={a} idx={i} />)}
        </div>
      </section>
  ...
  return (
    <div className="card-surface rounded-xl p-5">
      ...
      <button onClick={onCta} className="mt-4 inline-flex items-center gap-1 text-[12.5px] text-accent-500 hover:text-accent-600 transition">
        {cta} <Icons.ArrowUpRight size={13} />
      </button>
    </div>
  );
}
